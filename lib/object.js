...
export class RubyObject {
    constructor(type, collect=true) {
        ...
        if (this.type === undefined) {
            ...
            if (collect) {
                // TODO: Don't copy the instance methods!
    ...
    methods(name, ctx) {
        ...
        try {
            ...
            if (type === null) {
                throw new exc.NoMethodError(`undefined method ${name} for ${this.name}`);
            }
            return type.methods(name, ctx);
        } catch (e) {
            if (!((e instanceof exc.NoType) || (e instanceof exc.NoMethodError))) {
                throw e;
            }
